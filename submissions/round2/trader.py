...
class Trader:
    ...
    def bid(self) -> int:
        """
        MAF auction bid for Round 2.

        Derivation (see writeups/round2_notes.md):
        - Empirical V (marginal value of +25% quote access) ≈ 358 XIRECs
          (backtest proxy: passive_share 0.5 → 0.625).
        - Why small: PEPPER dominates PnL (~240k/271k) and sits at +80 limit
          100% of ticks, so MAF adds nothing on PEPPER. MAF upside is limited
          to OSMIUM's small contribution (~32k over 3 days).
        - Crowd: low/base/high scenarios; bid 100 has positive EV in all three.
          Max downside = -100 (if V is actually 0).
        """
        return 100
    ...
    def _trade_osmium(self, od: OrderDepth, position: int, timestamp: int, td: dict) -> List[Order]:
        ...
        # v2 change: coefficient raised from 3/80 to 6/80 (EDA Q5 showed stuck-at--80
        # was costing PnL on day 1; stronger skew pulls reservation harder toward flat).
        res = FAIR - position * (6.0 / 80.0) + res_shift

        buy_cap = LIMITS["ASH_COATED_OSMIUM"] - position
        sell_cap = LIMITS["ASH_COATED_OSMIUM"] + position

        # v3 change: ASYMMETRIC hard inventory band. EDA Q5 showed Long@+80 episodes are
        # always favorable (mean mid move +3.5 to +12.8 over next 500 ticks) while
        # Short@-80 episodes are adverse (mean -31.30 on day 1). So we keep the -60
        # short-side cap (protects against the real failure mode) but drop the +60
        # long-side cap (keeps the favorable mean-reversion edge on stuck-long episodes).
        SHORT_HARD_BAND = 60
        if position <= -SHORT_HARD_BAND:
            sell_cap = 0
