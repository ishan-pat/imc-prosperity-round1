...
class Trader:
    ...
    def _trade_osmium(self, od: OrderDepth, position: int, timestamp: int, td: dict) -> List[Order]:
        ...
        # v2 change: hard inventory band at |pos| >= 60 — stop adding to the side that
        # would push us further toward the ±80 limit. Keep the opposite side active so
        # the strategy can still flatten.
        HARD_BAND = 60
        if position >= HARD_BAND:
            buy_cap = 0
        if position <= -HARD_BAND:
            sell_cap = 0
